...
public class GenerateSquareWave {
    ...
    * @param max 係数a
        */
    public List<Double> GenerateSquareWave(double coefficient, double max) {
        List<Double> results = new ArrayList<Double>();
    ...
    */
    ...
    public void init() {
        ...
        drawDataListR = generateRadianList(2, reso);
        drawDataListX = generateTime(2, freq);
        drawDataListY = GenerateSquareWave(1.0, 1.0);

        int count = 0;
        List<double> allDrawDataSquareWave = new ArrayList<>();
        List<double> sumAllDrawData = new ArrayList<>();
        for (double j = 1; j <= 1001; j++) {
            if (j % 2 = 1 && j != 1) {
                allDrawDataSquareWave.add(j, GenerateSquareWave(j, 1 / j););
                sumAllDrawData.add(j, allDrawDataSquareWave(j));
                count++;
            }
        }

        for(int k = 0; k < drawDataListY.size(); k++) {
            for (double j = 1; j <= 1001; j++) {
                if (j % 2 = 1 && j != 1) {
                    double Sum = sumAllDrawData(j).get(k);
                }
            }
            allDrawDataSquareWave(j, GenerateSquareWave(j, 1 / j);).set(j, Sum);
